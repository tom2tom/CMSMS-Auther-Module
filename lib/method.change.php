...
//MESSAGE - 'Nothing is required for this'
 ...
 case self::LOSEC:
	...
	if ($cdata['email_required']) {
		$one->title = $mod->Lang('title_email'); //TODO CURRENT
		$one->input = $this->GetInputText($id, 'login', 'auth1', '', 32, 96);
	} else {
		$one->title = $mod->Lang('title_login'); //TODO CURRENT
		$one->input = $this->GetInputText($id, 'login', 'auth1', '', 20, 32);
	}
	$elements[] = $one;

	$one = new \stdClass();
	$one->title = $mod->Lang('password'); //TODO CURRENT
	...
	$elements[] = $one;
 	...
 	$one = new \stdClass();
	if ($cdata['email_required']) {
		$one->title = $mod->Lang('title_email'); //TODO NEW
		$one->input = $this->GetInputText($id, 'login', 'auth3', '', 32, 96);
	} else {
		$one->title = $mod->Lang('title_login'); //TODO NEW
		$one->input = $this->GetInputText($id, 'login', 'auth3', '', 20, 32);
...
EOS;
	...
	$jsloads[] = <<<EOS
 $('#auth1,#auth3,#auth5').blur(function() {
