...
//    const HISEC = 2;
	...
	private function UniqueToken($length)
	{
		...
		return str_shuffle(substr($s1, 0, $length));
	}
...
*/
	...
	@token: optional session-data identifier generated by a previous iteration of this, default FALSE
	@preserve: optional assoc array of parameters to be passed back when the action is finished
	...
	[1] = FALSE
	*/
	public function GetPanel($context, $task, $handler, $withcancel=FALSE, $token=FALSE, $preserve=FALSE)
	{
...
$cdata['security_level'] = 3; //DEBUG
		...
		if ($token) {
			...
			$token = $this->UniqueToken(24); //cache value for later use
			$sdata = FALSE;
		}
...
function ajaxresponse(details, defltmsg, iserr) {
 var msg, \$el, ht;
...
function clearresponse() {
 var \$el = $('#authcontainer p.feedback');
 if (\$el.length > 0) {
  \$el.remove();
 }
}
EOS;
		$tabindex = 1; //CHECKME phase-specific?
		$elements1 = []; //phase-1 UI-elements accumulator
		$elements2 = []; //ditto for phase-2
		...
		$tplvars['elements1'] = $elements1;
		if ($elements2) {
			$tplvars['elements2'] = $elements2;
			$jsloads[] = <<<'EOS'
 $('#authelements #phase2').css('display','none');
EOS;
		}
		$tplvars['submitbtn'] =
...
{$hidden}
  ...
  <div id="authelements">
  {if !empty($elements1)}<div id="phase1">
  {foreach from=$elements1 item='elem'}
...
{if !empty($elem->posthtml)}{$elem->posthtml}{/if}
  ...
  {if !empty($elements2)}<div id="phase2">
  {foreach from=$elements2 item='elem'}
{if !empty($elem->prehtml)}{$elem->prehtml}{/if}
{if !empty($elem->title)}<p class="authtitle">{$elem->title}</p>{/if}
{if !empty($elem->input)}<div class="authinput">{$elem->input}</div>{/if}
{if !empty($elem->extra)}<div class="authtext">{$elem->extra}</div>{/if}
